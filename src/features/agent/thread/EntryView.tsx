...
import StreamingText from "@/components/agent-ui/beautiful-ui/StreamingText";
...
export const EntryView = memo(function EntryView({
  ...
  switch (entry.kind) {
          ...
          {groupedChunks.map((g, i) =>
            g.type === "thought" ? (
              ...
              <MessageContextMenu key={i} textContent={g.text}>
                <StreamingText loop={false} fill>
                  <div className="nex-assistant-prose">
    ...
    case "completed_plan": {
      ...
      return (
        <div className="max-w-[96%] rounded-[calc(var(--radius-md)+2px)] border border-[color:var(--hairline-soft)] bg-[var(--material-floating)] px-2.5 py-1.5 shadow-none">
          <div className="flex items-center gap-2 text-xs text-[var(--text-secondary)] mb-1">
            <ListChecks size={14} />
            <span>
              {allCompleted ? "Completed Plan" : "Plan Snapshot"} —{" "}
              {entry.entries.length} steps
            </span>
          </div>

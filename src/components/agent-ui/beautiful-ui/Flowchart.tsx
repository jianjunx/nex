...
function Handle() {
  return (
    <svg
      ...
      height="16"
      viewBox="0 0 10 16"
      className="shrink-0 cursor-grab text-ink-3/70"
    >
      {[3, 8, 13].flatMap((y) => [
    ...
    >
      ...
      {items.map((item, i) => (
        ...
        >
          ...
          {item.tag && (
            <span className="shrink-0 text-[11px] text-ink-3">{item.tag}</span>
          )}
          <span
            className={`shrink-0 text-ink ${item.name === value ? "" : "invisible"}`}
          >
            <CheckIcon />
          </span>
        </button>
...
function FlowchartPreview({
  ...
  /* ── dragging ── */
  const onPointerDown =
    (node: StepNode) => (event: React.PointerEvent<HTMLDivElement>) => {
...
export default function Flowchart(
  props: NonNullable<Parameters<typeof FlowchartPreview>[0]> & {
  ...
  if (props.children !== undefined)
    return (
      <div
        data-agent-ui="Flowchart"
        className={`beautiful-ui nex-ui-Flowchart ${props.className ?? ""}`}
      >
        {props.children}
      </div>
    );
  return <FlowchartPreview {...props} />;
}

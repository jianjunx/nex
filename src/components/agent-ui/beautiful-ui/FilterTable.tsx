...
const FILTERS: {
  ...
  { key: "todo", label: "待处理", dot: "#f09a2f", count: 2 },
  { key: "progress", label: "进行中", dot: "#16a6c7", count: 2 },
  { key: "done", label: "已完成", dot: "#25a878", count: 1 },
];
...
const PILLS: Record<Status, { label: string; cls: string }> = {
  todo: { label: "待处理", cls: "filter-status-todo" },
  progress: { label: "进行中", cls: "filter-status-progress" },
  done: { label: "已完成", cls: "filter-status-done" },
};
          ...
          return (
            ...
            >
              ...
              >
                {f.key === "all"
                  ? rows.length
                  : rows.filter((row) => row.status === f.key).length}
              </span>
            </button>
          );
        })}

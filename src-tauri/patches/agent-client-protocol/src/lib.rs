...
impl ClientSideConnection {
    ...
    ///
    /// Used for newer ACP methods (e.g. `session/set_config_option`) that are not
    /// yet modeled in this crate's typed `ClientRequest` enum. `ExtRequest`
    /// serializes transparently as its `params` only, so the wire shape is correct.
    pub async fn request_raw(
        ...
        self.conn
            .request(
                method,
                Some(ClientRequest::ExtMethodRequest(ExtRequest {
                    method: method.into(),
                    params: raw.into(),
                })),
            )
...
impl Side for ClientSide {
    ...
    fn decode_request(method: &str, params: Option<&RawValue>) -> Result<AgentRequest> {
        ...
        match method {
            ...
            _ => {
                // ACP extensions are `_`-prefixed; Cursor uses unprefixed `cursor/*`.
                // Accept both so blocking extension methods reach `Client::ext_method`.
                let custom_method = method.strip_prefix('_').unwrap_or(method);
        ...
        match method {
            m if m == CLIENT_METHOD_NAMES.session_update => {
                match serde_json::from_str(params.get()) {
                    Ok(n) => Ok(AgentNotification::SessionNotification(n)),
                    Err(_) => {
                        // Forward unknown sessionUpdate variants (e.g. config_option_update
                        // on older schema) as an extension notification so clients can
                        // still consume them as raw JSON.

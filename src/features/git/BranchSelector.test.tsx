...
beforeEach(() => {
  ...
  gitState = {
    branches: [
      { name: "main", isHead: true, isRemote: false, ahead: 0, behind: 0, tipTime: 100 },
      { name: "feature", isHead: false, isRemote: false, ahead: null, behind: null, tipTime: 200 },
      { name: "origin/main", isHead: false, isRemote: true, ahead: null, behind: null, tipTime: 150 },
    ],
    opRunning: null,
...
describe("BranchSelector", () => {
  ...
  it("sorts local branches by tipTime descending (newest first)", () => {
    render(<BranchSelector projectPath="/p" open onOpenChange={() => {}} />);
    const locals = ["feature", "main"].map((n) => screen.getByTestId(`branch-${n}`));
    // feature (200) before main (100) in document order among local rows
    expect(locals[0].compareDocumentPosition(locals[1]) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  });

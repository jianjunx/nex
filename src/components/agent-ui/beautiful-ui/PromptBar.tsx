      ...
      <div ref={composerAnchorRef} className="relative">
        ...
        >
          ...
          {attachments.length > 0 && (
            <div
              className={`flex flex-wrap gap-1.5 pt-0.5 ${pill ? "px-1" : "px-0.5"}`}
            >
...
export default function PromptBar(
  props: NonNullable<Parameters<typeof PromptBarPreview>[0]> & {
  ...
  if (props.children !== undefined)
    return (
      <div
        data-agent-ui="PromptBar"
        className={`beautiful-ui nex-ui-PromptBar ${props.className ?? ""}`}
      >
        {props.children}
      </div>
    );
  return <PromptBarPreview {...props} />;
}

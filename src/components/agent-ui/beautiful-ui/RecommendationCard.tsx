...
import { Button, type ButtonVariant } from "@/components/agent-ui/atoms/Button";
import { EntityChip } from "@/components/agent-ui/atoms/EntityChip";
import { ValuePill } from "@/components/agent-ui/atoms/ValuePill";
...
const OPTIONS: RecommendationOption[] = [
  ...
  },
  {
    ...
    body: (
      <>
        Fall back to a{" "}
        <span className="font-medium text-ink">full restock</span> across every
        SKU.
      </>
    ),
...
export default function RecommendationCard(
  props: NonNullable<Parameters<typeof RecommendationCardPreview>[0]> & {
  ...
  if (props.children !== undefined)
    return (
      <div
        data-agent-ui="RecommendationCard"
        className={`beautiful-ui nex-ui-RecommendationCard ${props.className ?? ""}`}
      >
        {props.children}
      </div>
    );
  return <RecommendationCardPreview {...props} />;
}

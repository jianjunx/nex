...
export default function FineTuneCard({
  ...
  const [internalValues, setValues] = useState<Record<string, number>>(() =>
  ...
  return (
    <div className="relative w-full max-w-60 rounded-card bg-surface shadow-raised">
      ...
      <div
        hidden={fieldsOnly}
        className="primitive-card-footer flex items-center justify-between"
      >
                ...
                {options.map((item) => (
                  <button
                    ...
                    className={`relative z-10 flex h-6.5 w-full items-center rounded-[6px] px-2 text-left text-[12.5px] text-ink ${
                      item === typeValue
                        ? "bg-field group-hover/glide-menu:bg-transparent"
                        : ""
                    }`}
                  >
                    {item}

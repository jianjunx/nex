...
### 前端界面风格（Beautiful UI，新组件库统一标准）

全项目使用 agent-ui / Beautiful UI 风格，弃用旧 macOS Pro 玻璃视觉。
样式权威入口是 src/styles/globals.css，组件适配位于 src/styles/agent-ui.css。
既有 material-* 命名继续作为语义接口，但表示不透明的中性表面，不表示玻璃。

- 共享深浅中性色板，按钮主色为 ink / surface；错误、警告、成功保持语义色。
- 控件圆角 8px，卡片/弹窗 14px，菜单项 6px；正文以 13px 为基准。
- 禁止新增 glass-* 变量、背景模糊、内高光、发光描边和大块蓝紫色选中背景。
- 优先修改 components/ui 共享原语；业务组件使用 material-*、hairline-*、
  overlay-*、text-* 变量，不另造主题或写固定深浅色。
- 菜单、弹窗采用不透明浮层、细边界和克制外阴影；工作面不叠加阴影。
- 焦点必须可见；反馈使用短促颜色/透明度变化，禁止按下缩放和弹簧动效。
- 保留 reduced-motion、contrast、键盘操作、焦点约束和三个平台支持。
- 组件库演示数据/计时器不得驱动真实 Agent 进度、审批或消息。
- 全局视觉修改必须验证主题、菜单、弹窗、设置、文件、Git、搜索和终端。

...
vi.mock("./sections/NexAgentSection", () => ({ NexAgentSection: () => <div data-testid="sec">Nex 智能体</div> }));
...
describe("SettingsDialog section targeting", () => {
  ...
  it("consumes a one-shot settingsSection on open", () => {
    useUiStore.setState({ settingsSection: "nex-agent" });
    render(<SettingsDialog />);
    expect(screen.getByTestId("sec").textContent).toBe("Nex 智能体");
  ...
  it("a plain reopen returns to appearance even after the tab was changed (R2)", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Nex 智能体" }));
    expect(screen.getByTestId("sec").textContent).toBe("Nex 智能体");

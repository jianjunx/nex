...
import { Shimmer } from "@/components/agent-ui/atoms/Shimmer";
import { StreamText } from "@/components/agent-ui/atoms/StreamText";
...
function SelectionActionsPreview({
  ...
  for (const item of [...actions.primary, ...actions.more]) {
    if (item.action && item.busyLabel)
      busyLabelMap[item.action] = item.busyLabel;
  }
                  ...
                  >
                    ...
                    {actions.primary.map((item) => (
                      <Button
                        ...
                        className="shrink-0"
                        onClick={
                          item.action ? () => run(item.action!) : undefined
                        }
                      >
                        {item.icon}
                        {item.id}
...
export default function SelectionActions(
  props: NonNullable<Parameters<typeof SelectionActionsPreview>[0]> & {
  ...
  if (props.children !== undefined)
    return (
      <div
        data-agent-ui="SelectionActions"
        className={`beautiful-ui nex-ui-SelectionActions ${props.className ?? ""}`}
      >
        {props.children}
      </div>
    );
  return <SelectionActionsPreview {...props} />;
}
